...
client/                          → React 19 SPA (Vite)
  src/
    ...
    lib/
      ...
      authorAvatars.ts            → Static author avatar URL map
...
| `author_profiles` | Author enrichment | `authorName`, `bio`, `avatarUrl`, `s3AvatarUrl`, `avatarSource`, `websiteUrl`, `twitterUrl`, `linkedinUrl` |
...
The author avatar pipeline is implemented in `server/lib/authorAvatars/waterfall.ts`. It tries each tier in order, stopping at the first success:
...
**Tier 5 (Replicate AI Portrait)** uses Black Forest Labs' `flux-schnell` model to generate a professional corporate headshot. The prompt is gender-aware (using name-based heuristics) and produces a 1:1 aspect ratio WebP image. AI-generated portraits are tagged with `avatarSource = 'ai'` and `isAiGenerated = true`.

**After finding a photo:** The waterfall uploads it to S3 via `uploadAvatarToS3()`, storing the stable CDN URL in `s3AvatarUrl`.
...
Users can manually upload author avatars through the `AvatarUpload` component:
...
6. **Persist** the S3 URL in `author_profiles.s3AvatarUrl`
