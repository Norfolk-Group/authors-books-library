 ...
 *
 * Platforms supported:
 *   website, businessWebsite, youtube, twitter/X, linkedin, substack,
...
interface PlatformDef {
  key: string;
  label: string;
  color: string;          // Tailwind bg class for pill
  textColor: string;      // Tailwind text class
  icon: React.ReactNode;  // SVG or lucide icon
}

// Inline SVG logos for platforms that have distinct brand icons
const YouTubeIcon = () => (
...
interface PlatformPillsProps {
  links: PlatformLinks;
  /** Maximum number of pills to display before collapsing. Default: 6 */
  maxVisible?: number;
...
export function PlatformPills({
  ...
  const visible = activePlatforms.slice(0, maxVisible);
  const overflow = activePlatforms.length - visible.length;
 ...
 * Useful for showing a "N platforms" indicator without rendering the full pills.
 */
